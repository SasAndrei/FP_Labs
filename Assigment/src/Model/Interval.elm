...
lengthToString: Maybe (Int, Int) -> String
lengthToString mby =
    case mby of
        Just (a, b) -> "text " ++ String.fromInt(a) ++ ",text " ++ String.fromInt(b)
        Nothing -> "No length"
...
compare (Interval intA) (Interval intB) =
    case (intA.end, intB.end) of
        (Nothing, Nothing) -> Date.compare  intA.start intB.start
        (Nothing, Just _) -> chainCompare GT (Date.compare intA.start intB.start)
        (Just _, Nothing) -> chainCompare LT (Date.compare  intA.start intB.start)
        (Just a, Just b) -> chainCompare (Date.compare a b) (Date.compare  intA.start intB.start) 


intervalend: Interval->String
intervalend (Interval int) = 
    case int.end of
        Just date -> 
            case Date.month date of
                Nothing -> (String.fromInt (year date))
                Just x ->  (String.fromInt (year date) ++ (Date.monthToString x))
        Nothing -> "Present"


intervalstart: Interval->String
intervalstart (Interval int) = 
    case Date.month int.start of
        Nothing -> (String.fromInt (year int.start))
        Just x ->  (String.fromInt (year int.start) ++ (Date.monthToString x))


view : Interval -> Html msg
view interval =
    div [class "interval"] [
        p [class "interval-start"] [text (intervalstart interval)],
        p [class "interval-end"][text (intervalend interval)],
        p[class "interval-length"][text (lengthToString (length interval))]]
