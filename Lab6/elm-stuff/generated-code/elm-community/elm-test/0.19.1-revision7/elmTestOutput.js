...
var $author$project$RecipeView$brownies = {
	ingredients: _List_fromArray(
		['200 g quality dark chocolate (70%)', '250 g unsalted butter', '75 g dried sour cherries , optional', '50 g chopped nuts , optional', '80 g quality cocoa powder', '65 g plain flour', '1 teaspoon baking powder', '360 g caster sugar', '4 large free-range eggs']),
	linkToOriginal: 'http://www.jamieoliver.com/recipes/chocolate-recipes/bloomin-brilliant-brownies',
	method: _List_fromArray(
		['Preheat the oven to 180C/350F/gas 4. \r\n         Line a 24cm square baking tin with greaseproof paper.', 'Snap the chocolate into a large bowl, \r\n         add the butter and place over a pan of simmering water, \r\n         until melted, stirring regularly. \r\n         Stir through the cherries and nuts (if using).', 'Sift the cocoa powder and flour into a separate bowl, \r\n         add the baking powder and sugar, then mix together.', 'Add the dry ingredients to the chocolate, \r\n         cherry and nut mixture and stir together well. Beat the eggs, \r\n         then mix in until you have a silky consistency.', 'Pour the brownie mix into the baking tin, \r\n         and place in the oven for around 25 minutes. \r\n         You don\'t want to overcook them so, unlike cakes, you don\'t want \r\n         a skewer to come out clean - the brownies should be slightly\r\n         springy on the outside but still gooey in the middle.']),
...
var $author$project$RecipeView$recipeView = function (recipe) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_Utils_ap(
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$href(recipe.linkToOriginal)
								]),
							_List_fromArray(
								[
					...
					A2(
						...
						function (ingr) {
							return A2(
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class('ingredient ')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text(ingr)
									]));
						},
						recipe.ingredients)),
					A2(
...
var $author$project$RecipeTests$atLeastOneIngredient = A2(
	$elm_explorations$test$Test$test,
	'each recipe contains at least one ingredient',
	function (_v0) {
		return A2(
			...
			A2(
				...
				A2(
					...
					_List_fromArray(
						[
							$elm_explorations$test$Test$Html$Selector$tag('ul')
						]),
					$elm_explorations$test$Test$Html$Query$fromHtml(
						$author$project$RecipeView$recipeView($author$project$RecipeView$brownies)))));
	});
...
var $author$project$RecipeTests$atLeastOneIngredientClass = A2(
	$elm_explorations$test$Test$test,
	'each recipe contains at least one ingredient (using classes)',
	function (_v0) {
		return A2(
			$elm_explorations$test$Test$Html$Query$count,
			$elm_explorations$test$Expect$atLeast(1),
			A2(
				...
				_List_fromArray(
					[
						$elm_explorations$test$Test$Html$Selector$class('ingredient')
					]),
				$elm_explorations$test$Test$Html$Query$fromHtml(
					$author$project$RecipeView$recipeView($author$project$RecipeView$brownies))));
	});
...
var $elm_explorations$test$Test$Html$Selector$disabled = $elm_explorations$test$Test$Html$Selector$Internal$namedBoolAttr('disabled');
...
var $author$project$CounterClass$Decrement = {$: 'Decrement'};
var $author$project$CounterClass$Increment = {$: 'Increment'};
...
var $author$project$CounterClass$redText = function (c) {
	return c ? _List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'color', 'red')
		]) : _List_Nil;
};
var $author$project$CounterClass$view = function (model) {
	var bigFont = A2($elm$html$Html$Attributes$style, 'font-size', '20pt');
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						bigFont,
						$elm$html$Html$Events$onClick($author$project$CounterClass$Increment),
						$elm$html$Html$Attributes$disabled(model >= 10)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('+')
				...
				_Utils_ap(
					_List_fromArray(
						[bigFont]),
					$author$project$CounterClass$redText(
						(_Utils_cmp(model, -8) < 1) || (model >= 8))),
				_List_fromArray(
					[
						$elm$html$Html$text(
				...
				_List_fromArray(
					[
						bigFont,
						$elm$html$Html$Events$onClick($author$project$CounterClass$Decrement),
						$elm$html$Html$Attributes$disabled(
						_Utils_cmp(model, -10) < 1)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('-')
...
var $author$project$CounterTests$buttonDisabledOver10 = A2(
	$elm_explorations$test$Test$test,
	'when the counter has value 10 the + button is disabled',
	function (_v0) {
		return A2(
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$elm_explorations$test$Test$Html$Selector$disabled(true)
					]),
				$elm_explorations$test$Test$Html$Query$fromHtml(
					$author$project$CounterClass$view(10))));
	});
...
var $elm_explorations$test$Test$Internal$Skipped = function (a) {
	return {__elmTestSymbol: __elmTestSymbol, $: 'Skipped', a: a};
};
var $elm_explorations$test$Test$skip = $elm_explorations$test$Test$Internal$Skipped;
var $author$project$RecipeTests$eachIngredientHasClassIngredient = $elm_explorations$test$Test$skip(
	A2(
		$elm_explorations$test$Test$test,
		'each ingredient has class ingredient',
		function (_v0) {
			return A2(
...
var $author$project$CoinFlip$Model = F2(
	function (currentFlip, flips) {
		return {currentFlip: currentFlip, flips: flips};
	});
var $author$project$CoinFlip$initModel = A2($author$project$CoinFlip$Model, $elm$core$Maybe$Nothing, _List_Nil);
var $author$project$CoinFlip$Flip = {$: 'Flip'};
var $author$project$CoinFlip$coinToString = function (coin) {
	if (coin.$ === 'Heads') {
		return 'h';
	} else {
		return 't';
...
var $author$project$CoinFlip$viewCoin = function (coin) {
	var name = $author$project$CoinFlip$coinToString(coin);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$CoinFlip$view = function (model) {
	var flips = A2(
		...
		A2(
			...
			' ',
			A2($elm$core$List$map, $author$project$CoinFlip$coinToString, model.flips)));
	var currentFlip = A2(
		$elm$core$Maybe$withDefault,
		$elm$html$Html$text('Press the flip button to get started'),
		A2($elm$core$Maybe$map, $author$project$CoinFlip$viewCoin, model.currentFlip));
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Events$onClick($author$project$CoinFlip$Flip)
					]),
				_List_fromArray(
					[
				...
				currentFlip,
				A2($elm$html$Html$div, _List_Nil, flips)
			]));
};
var $author$project$CoinFlipTests$initialViewTest = A2(
	$elm_explorations$test$Test$test,
	'When the app is started, there should be text saying: Press the..',
	function (_v0) {
		return A2(
			$elm_explorations$test$Test$Html$Query$has,
			_List_fromArray(
				[
					$elm_explorations$test$Test$Html$Selector$text('Press the flip button to get started')
				]),
			$elm_explorations$test$Test$Html$Query$fromHtml(
				$author$project$CoinFlip$view($author$project$CoinFlip$initModel)));
	});
...
var $author$project$CounterTests$viewContainsTheCurrentCount = A2(
	$elm_explorations$test$Test$test,
	'view contains the current count',
	function (_v0) {
		return A2(
			$elm_explorations$test$Test$Html$Query$has,
			_List_fromArray(
			...
			$elm_explorations$test$Test$Html$Query$fromHtml(
				$author$project$CounterClass$view(0)));
	});
...
var $author$project$CounterTests$viewHasTwoButtons = A2(
	$elm_explorations$test$Test$test,
	'view contains two buttons',
	function (_v0) {
		return A2(
			$elm_explorations$test$Test$Html$Query$count,
			$elm_explorations$test$Expect$equal(2),
			A2(
				$elm_explorations$test$Test$Html$Query$findAll,
				_List_fromArray(
					[
...
var $author$project$Test$Generated$Main$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		...
		paths: _List_fromArray(
			['d:\\teaching\\2021_2022\\fp\\L06\\tests\\CoinFlipTests.elm', 'd:\\teaching\\2021_2022\\fp\\L06\\tests\\CounterTests.elm', 'd:\\teaching\\2021_2022\\fp\\L06\\tests\\RecipeTests.elm']),
		processes: 4,
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$CoinFlipTests$initialViewTest)
				])),
			_Utils_Tuple2(
			'CounterTests',
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$CounterTests$viewHasTwoButtons),
					$author$project$Test$Runner$Node$check($author$project$CounterTests$viewContainsTheCurrentCount),
					$author$project$Test$Runner$Node$check($author$project$CounterTests$buttonDisabledOver10)
				])),
			_Utils_Tuple2(
			'RecipeTests',
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$RecipeTests$atLeastOneIngredient),
					$author$project$Test$Runner$Node$check($author$project$RecipeTests$atLeastOneIngredientClass),
					$author$project$Test$Runner$Node$check($author$project$RecipeTests$eachIngredientHasClassIngredient)
				]))
		]));
_Platform_export({'Test':{'Generated':{'Main':{'init':$author$project$Test$Generated$Main$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "\\\\.\\pipe\\elm_test-6112-1";
var net = require('net'),
